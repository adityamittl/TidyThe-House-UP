...
# total batches of data = 12

train_ds = dataset.take(9) # 80% of 12
test_ds = dataset.skip(9) # remaining 20%
val_ds = test_ds.take(1) # 10% of 12
test_ds = test_ds.skip(1) # remaining 2
...
checkpointer = ModelCheckpoint(filepath="roomcleanliness_weights.h5", verbose=1, save_best_only=True)
...
with open("roomcleanliness_model.json","w") as json_file:
    json_file.write(model_json)

# Load pretrained model (best saved one)
with open('roomcleanliness_model.json', 'r') as json_file:
    json_savedModel= json_file.read()
# load the model  
model = tf.keras.models.model_from_json(json_savedModel)
model.load_weights('roomcleanliness_weights.h5')
model.compile(loss = 'sparse_categorical_crossentropy', optimizer=opt, metrics= ["accuracy"])
